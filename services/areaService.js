...
/**
 * Get all objects for a specific area ID
 * @param {string} bucketName - Name of the bucket
 * @param {string} areaId - Area ID to retrieve data for
 * @returns {Promise<Array>} - Array of objects with their data
 */
exports.getObjectsByAreaId = async (bucketName, areaId) => {
  try {
    ...
    // Create the prefix pattern for this area
    const prefix = `${areaId}/`;
...
/**
 * Process area energy data and organize by date
 ...
 */
exports.processAreaData = (minioObjects, targetDate = null, timeFormat = "hour", latestOnly = true) => {
    ...
    validFiles.forEach(file => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        headers.forEach((header, index) => {
          ...
          if (['total_electricity_usage_kwh'].includes(header)) {
        ...
        // Standardize field names - if we have total_electricity_usage_kwh but not electricity_usage_kwh
        if (record.total_electricity_usage_kwh !== undefined && record.electricity_usage_kwh === undefined) {
          record.electricity_usage_kwh = record.total_electricity_usage_kwh;
        }
    ...
    const result = {
      area_id: allRecords.length > 0 ? allRecords[0].device_id : null,
...
/**
 * Get and process area data for a specific date
 * @param {string} bucketName - Name of the bucket
 * @param {string} areaId - Area ID to retrieve data for
 * @param {string} targetDate - Date in format 'YYYY-MM-DD' or null for all dates
 * @param {string} timeFormat - Group by 'timestamp', 'hour', 'date', 'month', or 'year'
 * @param {boolean|string} latestOnly - If true/string 'true', only keep latest record for each time group
 * @returns {Promise<Object>} - Processed data in requested format
 */
exports.getAndProcessAreaData = async (bucketName, areaId, targetDate = null, timeFormat = 'hour', latestOnly = false) => {
  try {
    // Get all objects for the area
    const objects = await exports.getObjectsByAreaId(bucketName, areaId);

    // Process the data
    const processedData = exports.processAreaData(objects, targetDate, timeFormat, latestOnly);
